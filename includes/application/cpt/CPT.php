...
class WRB_CPT
{
	...
	{
		$this->id 				   = 'WRB_cpt';
		$this->label               = esc_attr__('BoilerPlate test CPY', WRB_DOMAIN);
		$this->labels 			   = new WRB_CPTLabels();
		$this->description         = esc_attr__('Some blank description for Boilerplate CPT', WRB_DOMAIN);
...
class WRB_CPT_builder
{

	function __construct()
	{
		$this->cpt = new WRB_CPT();
	}
	...
	{
		return new WRB_CPTLabels_Builder($this->cpt);
	}
}

require_once WRB_ACHIEVEMENTS_PATH_CPT . 'CPTArgs.php';
require_once WRB_ACHIEVEMENTS_PATH_CPT . 'create_cpt.php';

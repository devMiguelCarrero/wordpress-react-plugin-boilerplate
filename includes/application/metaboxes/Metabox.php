<?php
	...
	class WRB_Metabox {
		...
		function __construct() {
			...
			$this->id = 'WRB_metabox';
	        $this->title = esc_attr__('WRB custom Metabox', WRB_DOMAIN);
	        $this->cpt = 'post';
	        $this->position = 'side';
	        $this->priority = 'high';
	        $this->frontend = new WRB_Metabox_Front();
	        $this->save = new WRB_Metabox_Save();

		}

		public function factory_Input( $post ) {

			wp_nonce_field( WRB_DOMAIN , 'nonce-'.$this->id );
		...
		public function factory_Input_Date( $post ) {
			...
			?>
				<input type="<?php echo $this->frontend->type; ?>" name="<?php echo $this->id; ?>" class="widefat" value="<?php echo WRB_Timezone::instance()->getDateByUserTimezone( date( 'Y-m-d\TH:i' , strtotime($meta) ) ); ?>">
				<small><?php echo esc_html__( 'Timezone' , WRB_DOMAIN ); ?>: <?php echo WRB_Timezone::instance()->getUserTimezone(); ?></small>
			<?php
		...
		public function factory_Image_Upload( $post ) {
		    ...
		    if ( $image_id && get_post( $image_id ) ) {
		        ...
		        if ( ! empty( $thumbnail_html ) ) {
		            ...
		            $content .= '<p class="hide-if-no-js"><a href="javascript:;" class="admin-remove-image" data-id="upload-'. $this->id .'" data-title="'. $this->title .'" id="upload_'.$this->id .'_button" >' . esc_html__( 'Remove ', WRB_DOMAIN ) . $this->title .'</a></p>';
		        ...
		        $content .= '<p class="hide-if-no-js"><a data-id="upload-'. $this->id .'" title="' . esc_attr__( 'Set ', WRB_DOMAIN ) . $this->title . '" href="javascript:;" id="upload_'.$this->id .'_button" class="admin-upload-image" data-title="'. $this->title .'" data-uploader_title="' . esc_attr__( 'Choose an image', WRB_DOMAIN ) . '" data-uploader_button_text="' . esc_attr__( 'Set ' . $this->title , WRB_DOMAIN ) . '">' . esc_html__( 'Set ', WRB_DOMAIN ) . $this->title . '</a></p>';
		...
		public function factory_Radio_Image() {
        	...
        	$options = WRB_CPTHelper::instance()->getAllCPTById( $this->frontend->options['cpt'] , $this->frontend->options['quantity'] );
		...
		public function factory_Ajax_content() {
			...
			?>
				<div id="ajax-<?php echo $this->id; ?>" class="ajax-admin" data-ajax="<?php echo $this->frontend->ajaxCallback; ?>" data-ajax-onload="<?php echo $this->frontend->ajax_on_load; ?>" <?php if( $this->frontend->js_callback != false ) { echo 'data-js-callback=\''.$this->frontend->js_callback.'\''; } ?>>
					<p><?php echo esc_html__( 'Loading...' , WRB_DOMAIN ); ?></p>
				</div>
		...
		public function factory_single_react($post) {
            ...
            WRB_View::get( 'single-react' , [ 'id' => 'edit-' . $this->id, 'meta_id' => $this->id, 'value' => $post_meta ] );
        }
		...
		public function factory_Save_metabox( $post_id , $post ) {
	        ...
	        if ( ! wp_verify_nonce( $_POST['nonce-'.$this->id] , WRB_DOMAIN ) ) {
		        return $post_id;
		    }
		...
		public function factory_Save_Date_metabox( $post_id , $post ) {
			...
			$events_meta[$this->id] = WRB_Timezone::instance()->convertTimezoneToGMT( esc_attr( $_POST[$this->id] ) );
	...
	class WRB_Metabox_builder {

		function __construct() {

			$this->metabox = new WRB_Metabox();
		...
		public function setFrontEnd() {
			return new WRB_Metabox_Front_Builder($this->metabox);
		}

		public function setSave() {
			return new WRB_Metabox_Save_Builder($this->metabox);
		}
	...
	require_once WRB_ACHIEVEMENTS_PATH_METABOXES . 'FrontEnd.php';
	require_once WRB_ACHIEVEMENTS_PATH_METABOXES . 'Save.php';
	require_once WRB_ACHIEVEMENTS_PATH_METABOXES . 'set_metaboxes.php';